...
ECHO Please enter the path these binaries are located (no trailing slash needed) : 
SET /p mysqlBinPath="(Type '*' to use %MySQL%) : "
IF /i "%mysqlBinPath%"=="*" (SET mysqlBinPath=%MySQL%& SET mysqlPath=%MySQL%\mysql.exe)
IF EXIST %mysqlPath% (ECHO Binaries found !) ELSE (ECHO Binaries NOT found !)
ECHO.
...
SET /P ls_user="MySQL Username (default is '%ls_user%'): "
...
IF NOT "%ls_name%"=="%gs_name%" SET gs_nameb="%ls_name%"
...
SET cmdline="%mysqldumpPath%" --add-drop-table -h %ls_host% -u %ls_user% --password=%ls_pass% %ls_name% ^> "%backup%\loginserver_backup.sql" 2^> NUL
%cmdline%
IF %ERRORLEVEL% == 0 ECHO Backup finished.&ECHO.&GOTO ls_db_ok
:ls_error1
...
SET cmdline="%mysqlPath%" -h %ls_host% -u %ls_user% --password=%ls_pass% -e "CREATE DATABASE `%ls_name%` DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_unicode_ci" 2^> NUL
%cmdline%
IF %ERRORLEVEL% == 0 ECHO Database created successfully !&PAUSE>NUL&SET install_type=f&GOTO gs_backup
:gs_err2
...
SET cmdline="%mysqlPath%" -h %ls_host% -u %ls_user% --password=%ls_pass% -D %ls_name% ^< login_install.sql 2^> NUL
%cmdline%
IF %ERRORLEVEL% == 0 (
ECHO Loginserver tables were deleted !
ECHO.
ECHO Press any key to continue ...&PAUSE>NUL&GOTO gs_backup)
ECHO ERROR !
GOTO end
...
SET cmdline="%mysqlPath%" -h %ls_host% -u %ls_user% --password=%ls_pass% -e "ALTER DATABASE `%ls_name%` DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_unicode_ci" 2^> NUL
%cmdline%
SET cmdline=
ECHO Trying to make a backup of your gameserver database.
SET cmdline="%mysqldumpPath%" --add-drop-table -h %gs_host% -u %gs_user% --password=%gs_pass% %gs_name% ^> "%backup%\gameserver_backup.sql" 2^> NUL
%cmdline%
IF %ERRORLEVEL% == 0 ECHO Backup finished.&ECHO.&GOTO gs_db_ok
:gs_error1
...
SET cmdline="%mysqlPath%" -h %gs_host% -u %gs_user% --password=%gs_pass% -e "CREATE DATABASE `%gs_name%` DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_unicode_ci" 2^> NUL
%cmdline%
IF %ERRORLEVEL% == 0 (
ECHO Database created successfully !
ECHO.
ECHO Press any key to continue ...&PAUSE>NUL&GOTO upgrade_install
:gs_err2
...
SET cmdline="%mysqlPath%" -h %gs_host% -u %gs_user% --password=%gs_pass% -D %gs_name% ^< game_install.sql 2^> NUL
%cmdline%
IF %ERRORLEVEL% == 0 ECHO Gameserver tables were deleted.&ECHO.&ECHO Press any key to continue ...&PAUSE>NUL&GOTO upgrade_install
ECHO ERROR !
GOTO error_end
...
COLOR 07
CLS
SET cmdline="%mysqlPath%" -h %gs_host% -u %gs_user% --password=%gs_pass% -e "ALTER DATABASE `%gs_name%` DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_unicode_ci" 2^> NUL
%cmdline%
SET cmdline=
IF /i "%install_type%"=="f" (
...
ECHO %action% %~nx1
IF "%dest%"=="ls" SET cmdline="%mysqlPath%" -h %ls_host% -u %ls_user% --password=%ls_pass% -D %ls_name% ^< %1
IF "%dest%"=="gs" SET cmdline="%mysqlPath%" -h %gs_host% -u %gs_user% --password=%gs_pass% -D %gs_name% ^< %1
%cmdline%
IF NOT %ERRORLEVEL% == 0 CALL :sql_error %1
GOTO :eof
...
if  /i "%ntpebcak%"=="q "(call :error_end & exit)
goto ask_sql
...
FOR %%i IN (*.sql) DO ECHO "%mysqlPath%" -h %ls_host% -u %ls_user% --password=%ls_pass% -D %ls_name% ^< %%i >> temp.bat
CALL temp.bat> NUL
REM DEL temp.bat
CD %workdir%
...
CD ..\sql\game\updates
ECHO @ECHO off> temp.bat
FOR %%i IN (*.sql) DO ECHO "%mysqlPath%" -h %gs_host% -u %gs_user% --password=%gs_pass% -D %gs_name% ^< %%i >> temp.bat
CALL temp.bat> NUL
REM DEL temp.bat
CD %workdir%
PAUSE
...
ECHO.
ECHO 1 - Read the Naia Project
ECHO 		[ http://naia-project.niloo.fr/viewtopic.php?f=29&t=28 ]
ECHO.
ECHO 2 - Get support in our forums
ECHO 		[ http://naia-project.niloo.fr/viewforum.php?f=10 ]
ECHO.
ECHO Press any key to continue ...
PAUSE>NUL
...
ECHO.
ECHO Visit [ http://naia-project.niloo.fr ] for more info about the Naia project.
ECHO.
ECHO Press any key to continue ...
PAUSE>NUL