...
boost::mutex * locks[NUMBER_OF_GENERATORS];
CRandomMersenne * generators[NUMBER_OF_GENERATORS];
unsigned counter = 0;

unsigned generate_seed()
{
	unsigned mstime = get_ms_time();
	unsigned stime = unsigned (time(NULL));
	unsigned rnd[2];
	rnd[0] = rand() * rand() * rand();
	rnd[1] = rand() * rand() * rand();

	unsigned val = mstime ^ rnd[0];
	val += stime ^ rnd[1];
	return val;
}
...
{
	srand (get_ms_time());
	for (unsigned i = 0 ; i < NUMBER_OF_GENERATORS ; ++i)
	{
		generators[i] = new CRandomMersenne (generate_seed());
		locks[i] = new boost::mutex();
	}
}
...
unsigned RandomUInt(unsigned n)
{
	unsigned ret;
	unsigned c;
	for ( ; ; )
...
unsigned RandomUInt()
{
	...
	for ( ; ; )
	{
		...
		if(locks[c]->try_lock())
		{
			ret = generators[c]->IRandom (0, RAND_MAX);
			locks[c]->unlock();
			return ret;
		}
		++counter;
...
void CRandomMersenne::Init0 (unsigned seed)
{
...
void CRandomMersenne::RandomInit(unsigned seed)
{
	// Initialize and seed
	Init0 (seed);
...
void CRandomMersenne::RandomInitByArray (unsigned seeds[], int length)
{
...
unsigned CRandomMersenne::BRandom ()
{
	...
	if (mti >= MERS_N)
	{
		...
		const unsigned LOWER_MASK = (1LU << MERS_R) - 1 ;       // Lower MERS_R bits
		const unsigned UPPER_MASK = 0xFFFFFFFF << MERS_R ;      // Upper (32 - MERS_R) bits
		static const unsigned mag01[2] = {0, MERS_A};
...
double CRandomMersenne::Random ()
{
	...
	union {double f ; unsigned i[2];} convert;
	unsigned r = BRandom() ;               // Get 32 random bits
...
int CRandomMersenne::IRandomX (int min, int max)
{
	...
	unsigned longran ;                     // Random bits * interval
	unsigned iran ;                        // Longran / 2^32
	unsigned remainder ;                   // Longran % 2^32

	interval = unsigned (max - min + 1);
	if (interval != LastInterval)
	{
		// Interval length has changed. Must calculate rejection limit
		// Reject when remainder = 2^32 / interval * interval
		// RLimit will be 0 if interval is a power of 2. No rejection then
		RLimit = unsigned ((unsigned(1) << 32) / interval) * interval - 1;
		LastInterval = interval;
	}
	do { // Rejection loop
		longran  = unsigned (BRandom()) * interval;
...
unsigned Random::random (unsigned min, unsigned max)
{
	...
	for ( ; ; )
	{
		...
		if(locks[c]->try_lock())
		{
			ret = generators[c]->IRandom (min, max);
			locks[c]->unlock();
			return ret;
		}
		++counter;
